      ...
      {links.map((link) => (
        <div key={link.name}>
          <div className="group px-3 text-left md:cursor-pointer">
            ...
            >
              {link.name}
              {link?.submenu === true && link?.sublinks && (
                <>
                  <span className="inline text-xl md:hidden">
                    {heading === link.name ? <FaChevronUp size={12} /> : <FaChevronDown size={12} />}
                  </span>
                      ...
                      <div key={mysublinks.Head}>
                        ...
                        {mysublinks.sublink.map((slink) => (
                          <li key={slink.name} className="text-gray-600 my-2.5 text-sm">
                            <Link to={slink.link} className="hover:text-primary-700">
                              {slink.name}
                            </Link>
                          </li>
                        ))}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <span className="inline text-xl md:ml-2 md:mt-1">
                        {subHeading === slinks.Head ? <FaChevronUp size={10} /> : <FaChevronDown size={10} />}
                      </span>
                    </h1>
