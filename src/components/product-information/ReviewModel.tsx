...
const ReviewModal = ({ isOpen, onClose, onSubmit, selectedOrderItem }: ReviewModalProps) => {
  return (
    <Modal visible={isOpen} onCancel={onClose} footer={null} className="rounded-lg p-6">
        ...
        {({ setFieldValue, values }) => (
          <Form className="space-y-4">
            <div className="w-full">
              <p className="mt-2 text-lg font-medium">Đánh giá của bạn</p>
              <div className="mt-2 flex space-x-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    key={star}
                    className={`cursor-pointer transition duration-150 ease-in-out ${
                      star <= values.rates ? "text-yellow-400" : "text-gray-300"
                    }`}
                    size={24}
            ...
            <div>
              <label className="mb-1 block text-lg font-medium" htmlFor="content">
                Đánh giá sản phẩm
              </label>
            ...
            <label className="mb-2 block text-lg font-medium">Hình ảnh sản phẩm</label>
            ...
            <div className="mt-6 flex justify-center">
              <Button type="submit" text="Gửi Đánh Giá" className="w-full max-w-xs" />
            </div>
          </Form>
        )}
      </Formik>
